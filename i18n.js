 ...
 */
  ...
  var I18N = (function () {
    ...
    var DICT = {
      ...
      /* KV-08 (#173). Klucz keepalived_virtual_router_id: generator wypisuje go jawnie,
         a przy imporcie zgłasza jego brak — kontrakt jak przy KV-12a. */
      /* Errata importu (#182): nazwa klucza, której kolla-ansible nie zna, wypisywana
         przez to narzędzie od v0.1 do v0.4. */
      "g.import.notUpstream":      "<code>{key}</code> is not a Kolla-Ansible variable and Kolla ignores it. Versions {versions} of this generator wrote it.",
      "g.import.notUpstream.hint": "The working key is <code>{correct}</code>. This import did not change your file: set the toggle to write the correct key on export, and remove the dead line yourself.",
      "g.hint.vrid":               "1–255, unique per environment on a shared L2 domain.",
      "g.ack.kv08":                "this L2 domain carries exactly one Kolla install",
      "g.ack.kv08.desc":           "This L2 domain carries exactly one Kolla install and no side-by-side redeploy is planned.",
      "g.yaml.kv08.comment":       "keepalived_virtual_router_id  - explicit on purpose: the upstream default is 51 in every install; change it when a second Kolla install shares this L2 domain, or the two VIPs will fight for master",
      "g.rule.kv08.msg":           "<code>keepalived_virtual_router_id</code> is absent, so keepalived runs with the upstream default 51. Import does not add missing keys.",
      "g.rule.kv08.hint":          "Every Kolla install defaults to 51. A second environment in the same L2 domain runs a second VRRP group with the same VRID; the VIP flaps between them and each HAProxy receives the other's traffic. Set an explicit, unique value, or acknowledge a deliberately isolated segment.",
      "g.rule.kv08.why":           "VRRP identifies a group by its router id; two groups with one id on one segment are one group as far as the protocol is concerned.",
      "g.rule.kv08.ack":           "This L2 domain carries exactly one Kolla install and no side-by-side redeploy is planned.",
      "g.d.vridRange":             "<code>keepalived_virtual_router_id</code>: <code>{value}</code> is outside 1–255; a VRRP router id is an eight-bit field and zero is reserved.",
      ...
      "g.d.kv05.same":             "The interface you named for Ceph traffic (<code>{stg}</code>) and <code>api_interface</code> (<code>{api}</code>) sit on the same device <code>{dev}</code>.",
      "g.d.kv05.inherits":         "You named no interface for Ceph traffic, so it follows <code>network_interface</code> (<code>{net}</code>) while <code>enable_hacluster</code> is on.",
      /* Errata importu, wariant „usunięte przez upstream" (#185) — odróżniony od
         wariantu „nazwa wymyślona", bo czytelnikowi należy się wydanie i to, czym
         ta zmienna była. */
      "g.import.removedUpstream":  "<code>{key}</code> was removed from Kolla-Ansible in {release} and is ignored. Versions {versions} of this generator wrote it.",
      "g.import.removedUpstream.hint": "It {note}. This import did not change your file; remove the dead line yourself.",
      ...
      /* VLAN -> network, 2026-09-02 (#65). Rozstrzygnięte kontraktem, jaki niesie samo
         repozytorium, a nie wyborem redakcyjnym: `live_migration_inbound_addr` jest ADRESEM
         i nie wymaga VLAN-u, a trzy pozostałe powierzchnie tej samej reguły mówiły „network"
         od początku — etykieta przełącznika (g.ack.kv06), jego adnotacja potwierdzenia
         (g.rule.kv06.ackNote) i komentarz WPISYWANY DO globals.yml użytkownika. Ostatni
         waży najwięcej: to jedyne miejsce, którego żaden deploy nie cofa. */
      "g.toggle.d16":              "A dedicated migration network configured through <code>live_migration_inbound_addr</code> in a nova-compute override.",
...
/* --- walidator: status wydania --- */
      ...
      /* OSTATNIE SIEDEM UJŚĆ — przeniesione 2026-09-02 (#58).
         ...
         Poprzednie partie były zakrojone gripem po `msg:`, `hint:` i `add(`, a te napisy
         siedziały pod `why:`, `fail:`, `what`, `todo` i w etykiecie przycisku — czyli pod
         nazwami pól, o których nikt nie pomyślał. Ta partia jest zakrojona po UJŚCIU:
         wszystko, co trafia do `msg`/`hint` findingu albo na ekran jako etykieta, idzie
         przez słownik niezależnie od tego, jak nazywa się pole w tabeli reguł.

         Cztery grupy kworum dzielą jedno zdanie `why` — jeden klucz, cztery użycia. */
      "v.q.why.control":       "Three independent quorum mechanisms ride on this one host list: Galera, RabbitMQ quorum queues and Corosync.",
      "v.q.why.own":           "The group carries its own membership instead of inheriting from <code>[control]</code>.",
      "v.q.fail.control":      "With two nodes, losing one puts Galera into non-Primary — the database goes read-only and every write API call returns 500. With four, losing two (one rack, one switch) kills quorum even though two healthy nodes remain. It surfaces at the first real failover, or during a rolling reboot while patching.",
      "v.q.fail.mariadb":      "Losing half the nodes puts the cluster into non-Primary — the database goes read-only.",
      "v.q.fail.rabbitmq":     "Quorum queues need a majority. Without one, publishing blocks and RPC calls between services time out.",
      "v.q.fail.hacluster":    "An even Corosync cluster cannot form a majority under a network partition — neither half takes over the resources.",
      "v.q.fail.etcd":         "Without a majority etcd goes read-only — services that lose contact with it cannot persist state.",
      "v.u.what.procedure":        "changes in release <code>{rel}</code> in a way that editing the file cannot settle",
      "v.u.todo.procedure":        "This needs an operational step on the running environment, not a YAML fix. " +
                                   "The tool cannot tell whether that step was taken — it would need the " +
                                   "deployment history, not the file. SCOPE.md explains why.",
      "v.u.what.renamed":          "is renamed to <code>{to}</code> in release <code>{rel}</code>",
      "v.u.what.circumstances":    "changes its circumstances in release <code>{rel}</code>",
      "v.u.todo.circumstances":    "The change does not block the upgrade, but it is worth noting.",
      "v.u.what.retired":          "stops being supported in release <code>{rel}</code>",
      "v.u.todo.retiredKey":       "Remove the key, or accept that it no longer does anything.",
      "v.u.todo.retiredGroup":     "Plan where the workload moves, then drop the group from the inventory.",
      "v.rep.lineButton":          "inventory: line {line}",
      "g.d.hostOs":                "Host systems supported in release <code>{rel}</code>: {list}.",
      "g.d.keyExplicit":           "<code>{key}</code> is set explicitly in the file — the value no longer depends on the release default ({label}).",
      "g.d.keyDefault":            "The file does not set <code>{key}</code> — the release default <code>{value}</code> applies ({label}).",
      "g.d.keyDefault.loaded":     "The loaded file does not set <code>{key}</code> — the release default <code>{value}</code> applies ({label}).",
      ...
      /* Kolejna partia przeniesiona z literałów walidatora (#56). Wartości przepisane
         ...
         NO-ADDRESS. CHILD-SELF nie miał wtedy podpowiedzi w kodzie i nie wolno jej było
         przy migracji wymyślać. Dostał ją później (#56) i nadal nie jest wymyślona: bierze
         v.f.groupCycle.hint, czyli podpowiedź findingu, od którego jest NIEROZDZIELNY —
         grupa wciągająca samą siebie JEST cyklem długości jeden, więc zdanie o odmowie
         Ansible jest o niej prawdziwe. Zobacz tools/golden/validator/group-child-self.ini:
         GROUP-CYCLE i CHILD-SELF zapalają się tam razem i nie da się ich rozdzielić. */
